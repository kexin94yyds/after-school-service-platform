...
一个面向毕业设计答辩、可运行且可一键验收的课后服务选课与教务监管系统。平台按开题报告设置学生、家长、教师和教务管理员四类角色及四个独立登录页面，贯通“学期与开课计划—排课—学生选课—个人课表—请假考勤—成绩评价—教务统计—操作审计”完整业务链。
...
| 角色 | 主要能力 |
| --- | --- |
| 学生 | 独立登录，按年级与时间筛选课程，选课/退选/原子改选，查看个人课表，提交未来课次请假，查看考勤、成绩和教师反馈 |
| 家长 | 独立登录，只读查看绑定子女的课表、请假、考勤、成绩与教师反馈，课程结束后分别评价课程和教师 |
| 教师 | 独立登录，仅查看本人授课任务、课次与学生名单，审核请假、登记考勤、申请纠错、填写教学记录并录入成绩与学习评价 |
| 教务管理员 | 独立登录，维护学期、账号、绑定、课程、开课计划、实际开班、排课调课、报名、纠错复核、成绩分布、Excel 与审计 |

主要业务闭环：

- 开课计划按 `草稿 → 已提交 → 已确认 → 已生效 → 已结束 → 已归档` 流转；启用前必须填写课程类型、计划课程/开班数、单班规模和师资人数，实际开班再落实教师、教室与服务周期。
- 开班发布必须关联标准学期、已备案计划和启用教室；后端与选课规则双重拒绝未备案开课，并校验教室容量、教师/教室/学生冲突。
- 学生在开课前提交请假，家长同步查看，教师批准后结果自动预填考勤名单。
- 已完成课次的考勤禁止直接覆盖；教师发起纠错、学校审批后，原值和新值写入不可变修订历史。
- 课次由教务排课结果生成，教师不能自行创建课次；调课同时校验教师、教室和已报名学生冲突并记录原因。
- 成绩采用 0–100 分并附教师学习评价；修改前值、新值、操作人和时间写入不可变历史。
- 课程、开班名单、行政班名单、课程绩效、成绩与统计均支持真实 XLSX；课程导入限制行列、大小和 XML/ZIP 安全边界。
- 所有成功写操作保存去敏审计元数据，不记录请求正文或密码；旧监管预警与整改数据保留用于历史兼容，但不进入四角色登录主流程。
...
教务管理员可在本校范围内纠正并取消单条报名；取消人和取消时间会保存在报名记录中。课次开始前不能登记考勤，提交时必须覆盖该课次的完整有效名单，完成后的考勤会锁定，避免无痕覆盖历史数据。
...
4. 构造填充 V15 演示库并升级到 V18/V21，另以空库验证生产 V18；校验 34 张业务表、分校学期、学生账号、成绩及修改历史外键。
5. 通过真实 HTTP Session 和 CSRF 跑通学生、家长、教师、教务管理员四角色权限，以及计划、排课、学生选课、课表、请假考勤、成绩评价、Excel 和审计闭环。
6. 用独立 JDK OOXML 解析器验证所有 XLSX（成绩不少于 2001 行），并重启后端验证 Flyway 幂等、演示回锚、关闭学期保护和数据库故障 readiness；退出时清理临时数据库和进程。
...
| 教师 | `teacher_wang` |
| 家长 | `parent_chen` |
| 第二所学校学生 | `student_zhao_xinghe` |
| 第二所学校教务管理员 | `school_admin_2` |
| 第二所学校教师 | `teacher_li` |
| 第二所学校家长 | `parent_zhao` |
...
- `db/migration/V16`：学生登录角色、学生账号关联、学生本人选课/请假与四角色审计约束
- `db/migration/V17`：0–100 分成绩、教师学习评价和不可变成绩修改历史
- `db/migration/V18`：停用历史监管登录角色，把共享学期无损拆分为学校独立学期，并用复合外键固化学校边界
- `db/demo/V20`：三名演示学生的独立登录账号
- `db/demo/V21`：演示成绩与教师学习评价

默认和 `prod` 模式只加载生产迁移，在空库上执行至 V18，不创建任何学校、人员或演示账号；两种模式保持 Flyway 严格顺序。常规构建产物在归档层再次排除 demo 迁移、profile 配置和刷新组件。

`demo-artifact` profile 额外加载 V4、V4.1、V8、V11、V15、V20、V21，并且仅在 demo JAR 的 `demo` profile 中开启 Flyway out-of-order。受 V18 学期学校化约束影响，演示库按 V15 填充后再升级到 V18/V21，不再把旧演示迁移逆序补入已经非空约束化的 V18 空库。演示迁移只能写入可随时丢弃的本机 `after_school_demo` 库。
...
首个学校与教务管理员通过一次性 systemd credential 原子创建；从教务入口成功登录并修改密码后立即删除 credential 和 drop-in，不要把初始化密码保留在环境文件中。
...
`prod` 模式严格解析并要求唯一的 `sslMode=VERIFY_IDENTITY`、`connectionTimeZone=+08:00` 和 `forceConnectionTimeZoneToSession=true`，默认启用 Secure 会话 Cookie，并关闭 OpenAPI 页面。应用会按“账号 + 来源地址”对连续登录失败执行指数退避，Nginx 同时提供登录/API 两级限流和安全响应头。数据库密码和初始教务管理员密码不得写入仓库。首次登录后可从右上角修改密码；修改或重置密码会使旧会话失效。

### 6. 备份、监控与回滚

[`scripts/backup-mysql.sh`](scripts/backup-mysql.sh) 把 MySQL 8.4 在线导出直接压缩并用 age 公钥加密，明文 SQL 不落盘；[`scripts/restore-mysql-backup.sh`](scripts/restore-mysql-backup.sh) 只允许恢复到尚不存在的新库，并要求成功 Flyway 历史至少包含 V18、19 张核心业务表、关键租户/报名/成绩历史外键和零条预警扫描运行孤儿记录。systemd timer 在每日 `00:15` 和 `12:15` 各运行一次，最多随机延迟 15 分钟；健康检查每 5 分钟验证 `MYSQL_DATABASE` 对应备份的 SHA-256 和新鲜度，18 小时未成功即告警，为 RPO 不超过 24 小时预留约 6 小时的修复窗口。
...
当前版本不包含支付、微信小程序/原生 App、AI 推荐、短信/微信等外部消息渠道和审批式候补队列；这些内容不在本次开题报告功能范围内。
